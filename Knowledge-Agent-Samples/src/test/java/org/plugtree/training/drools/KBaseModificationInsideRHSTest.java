...
public class KBaseModificationInsideRHSTest extends BaseKnowledgeAgentTest{
    ...
    public void test1() throws Exception{
        ...
        //kbase (see modifyRuleFileAndForceScan()). Even if Person present in 
        //the ksession matches the LHS of the new rule, the rule doesn't get 
        //activated
        ...
        //But we have 2 rules in the kbase
        assertEquals(2, kbase.getKnowledgePackage("org.drools").getRules().size());
        
        //Clear the global list
        executionsList.clear();
        
        //Insert a new Person that matches "Rule 2". A new activation is created.
    ...
    public void test2() throws Exception{
        //Create a Rule that, when fired, will modify the kbase adding a new 
        //rule and after that perform an update()
        rule1 = "";
        rule1 += "package org.drools\n";
        rule1 += "import org.drools.Person;\n";
        rule1 += "import org.plugtree.training.drools.KBaseModificationInsideRHSTest;\n";
        ...
        //Fire all the rules. The execution of Rule 1 will add a new rule to
        //kbase (see modifyRuleFileAndForceScan()). Even if Person present in 
        //the ksession matches the LHS of the new rule (and we are doing an
        //explicit call to update(Sp)), the new rule doesn't get 
        //activated
        ksession.fireAllRules();
    ...
    public void test3() throws Exception{
        ...
        ksession.fireAllRules();
        //No :(
        assertTrue(executionsList.isEmpty());
        
        //Now we have 2 Rules in the kbase
        assertEquals(2, kbase.getKnowledgePackage("org.drools").getRules().size());
